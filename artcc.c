...
struct _artcc_frequencies artcc[MAX_CHANNELS];
...
void artcc_main_run (void)
{
	if(rtx.lcd_update)
	{
		...
		if(rtx.vfo_A!=rtx.vfo_A_old || rtx.mode != rtx.mode_old)
		{
			...
			while(artcc[ch].frequenza)
			{
				itoa(artcc[ch].frequenza/1000,buf,10);
...
void calculate_emi(void)
{
	...
	for(ch=0;ch<MAX_CHANNELS;ch++)
	{
		if(artcc[ch].frequenza)
		{
			...
			if(ch<(MAX_CHANNELS-1) && artcc[ch+1].frequenza ==0)
			{
			...
			f_start   = artcc[ch].frequenza-artcc[ch].deviazione;
			f_stop    = artcc[ch].frequenza+artcc[ch].deviazione;
